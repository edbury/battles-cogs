...
log = logging.getLogger("red.plusplus")
...
class PlusPlus:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.scores = fileIO("data/plusplus/scores.json", "load")
        self.settings = fileIO("data/plusplus/settings.json", 'load')
    ...
    async def plusplus(self, ctx):
        """Checks a user's plusplus, requires @ mention
           Example: !plusplus @Red"""
        ...
        if self.scores.get(member.id, 0) != 0:
            ...
            await self.bot.say(member.name + " has no plusplus!")

    @commands.command(pass_context=True)
    async def plusplusboard(self, ctx):
        """plusplus leaderboard"""
        server = ctx.message.server
        member_ids = [m.id for m in server.members]
        plusplus_server_members = [key for key in self.scores.keys()
                                if key in member_ids]
        log.debug("plusplus server members:\n\t{}".format(
            plusplus_server_members))
    ...
    async def plusplusset(self, ctx):
        """Manage plusplus settings"""
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)
            return

    @plusplusset.command(pass_context=True, name="respond")
    async def _plusplusset_respond(self, ctx):
        ...
        fileIO('data/plusplus/settings.json', 'save', self.settings)
    ...
    async def check_for_score(self, message):
        ...
        for member in mentions:
            if member.id in first_word.lower():
                ...
                fileIO("data/plusplus/scores.json", "save", self.scores)
                return


def check_folder():
    if not os.path.exists("data/plusplus"):
        print("Creating data/plusplus folder...")
        os.makedirs("data/plusplus")
...
def check_file():
    ...
    f = "data/plusplus/scores.json"
    if not fileIO(f, "check"):
        print("Creating default plusplus's scores.json...")
        fileIO(f, "save", scores)

    f = "data/plusplus/settings.json"
    if not fileIO(f, "check"):
        print("Creating default plusplus's scores.json...")
...
def setup(bot):
    if tabulate is None:
        raise RuntimeError("Run `pip install tabulate` to use PlusPlus.")
    check_folder()
    check_file()
    n = PlusPlus(bot)
